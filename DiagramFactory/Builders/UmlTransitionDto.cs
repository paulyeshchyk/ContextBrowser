...
public readonly record struct UmlTransitionDto
{
    ...
    public static implicit operator (string Caller, string Callee, string Domain)(UmlTransitionDto value)
        => (value.CallerId, value.CalleeId, value.Domain);

    public static implicit operator UmlTransitionDto((string Caller, string Callee, string Domain) value)
        => new() { CallerId = value.Caller, CalleeId = value.Callee, Domain = value.Domain };

    public override int GetHashCode()
        => HashCode.Combine(CallerId, CalleeId, Domain);
