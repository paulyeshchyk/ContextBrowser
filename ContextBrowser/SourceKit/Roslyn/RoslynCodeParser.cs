...
// context: csharp, read
internal class RoslynCodeParser<TContext>
    where TContext : IContextWithReferences<TContext>
{
    ...
    {
        var cSharpSyntaxTree = CSharpSyntaxTree.ParseText(code, cancellationToken: CancellationToken.None) ?? throw new InvalidOperationException("Невозможно построить синтаксическое дерево");

        var root = cSharpSyntaxTree.GetCompilationUnitRoot(CancellationToken.None);

        var cSharpCompilation = CSharpCompilation.Create("Parser")
        ...
        var model = cSharpCompilation.GetSemanticModel(cSharpSyntaxTree);
        ...
        foreach(var availableSyntax in availableSyntaxies)
        {
            ...
            BuildMethodContexts(availableSyntax, options, contextOptions, cSharpSyntaxTree, model, nsName, typeContext);
        }
    ...
    protected void BuildMethodContexts(MemberDeclarationSyntax availableSyntax, RoslynCodeParserOptions options, RoslynContextParserOptions contextOptions, SyntaxTree cSharpSyntaxTree, SemanticModel semanticModel, string nsName, TContext? typeContext)
    {
        ...
        {
            BuildMethodContext(methodDeclarationSyntax, contextOptions, cSharpSyntaxTree, semanticModel, nsName, typeContext);
        }
    }
    ...
    private TContext? BuildTypeContext(MemberDeclarationSyntax availableSyntax, RoslynContextParserOptions contextOptions, SemanticModel model, string nsName)
    {
        ...
        var result = _factory.Create(default, kind, nsName, default, typeName, null);
    ...
    private TContext? BuildMethodContext(MethodDeclarationSyntax methodSyntax, RoslynContextParserOptions contextOptions, SyntaxTree cSharpSyntaxTree, SemanticModel model, string nsName, TContext? typeContext)
    {
        var methodName = methodSyntax.Identifier.Text;
        var methodSymbol = model.GetDeclaredSymbol(methodSyntax);//RoslynCodeParserMethodSymbolExtractor.ExtractMethodSymbol(cSharpSyntaxTree, methodName);
        ...
        var displayname = methodSymbol.ToDisplayString();

        var result = _factory.Create(typeContext, ContextInfoElementType.method, nsName, typeContext, methodName, displayname);
    ...
    {
        if(!collector.ByFullName.TryGetValue(methodContext.DisplayName, out var caller))
        {
            Console.WriteLine($"[MISS      ] {methodContext.DisplayName} not found in collector.ByFullName");
            return;
        }
