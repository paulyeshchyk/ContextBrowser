...
//context: build, html
public static class HtmlSequenceDomainPageBuilder
{
    //context: build, html
    public static void Build(string domain, List<ContextInfo> methods, PumlHtmlInjection? puml, ExportOptions _exportOptions)
    {
        var path = ExportPathBuilder.BuildPath(_exportOptions.Paths, ExportPathType.pages, $"composite_domain_{domain}.html");
        ...
        HtmlBuilderFactory.Html.With(writer, () =>
        {
            HtmlBuilderFactory.Head.With(writer, () =>
            {
                ...
                var embeddedScript = puml?.EmbeddedScript;
