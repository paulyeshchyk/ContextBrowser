...
//context: roslyn, read
public class RoslynInvocationSyntaxResolver : IInvocationSyntaxResolver
{
    ...
    public RoslynInvocationSyntaxResolver(IInvocationResolver<RoslynSyntaxTreeWrapper> semanticInvocationResolver, IAppLogger<AppLevel> logger, ICSharpInvocationSyntaxWrapperConverter invocationSyntaxWrapperConverter)
    {
        _logger = logger;
    ...
    internal async Task<ISyntaxWrapper?> GetSyntaxWrapperAsync(InvocationExpressionSyntax byInvocation, SemanticOptions options, CancellationToken cancellationToken)
    {
        ...
        var symbol = await RoslynInvocationSyntaxResolver.GetMethodSymbolAsync(invocationWrapper, invocationSemanticModel, _logger, cancellationToken).ConfigureAwait(false);
        return (symbol != null)
