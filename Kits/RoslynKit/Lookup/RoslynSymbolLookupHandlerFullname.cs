...
public class RoslynSymbolLookupHandlerFullname<TContext, TSemanticModel> : RoslynSymbolLookupHandlerBase<TContext, TSemanticModel>
...

{
    ...
    public RoslynSymbolLookupHandlerFullname(IContextCollector<TContext> collector, IAppLogger<AppLevel> logger)
        : base(logger)
    {
    ...
    public override async Task<TContext?> HandleAsync(ISyntaxWrapper symbolDto, CancellationToken cancellationToken)
    {
        ...
        return await base.HandleAsync(symbolDto, cancellationToken).ConfigureAwait(false);
    }
}
